...
它让你通过可复用的 **YAML 预设组合**，在 **apt**、**shell**、**Docker**、**k3s**、**Go** 等开发环境里统一切换并套用同一套本机 **HTTP(S)/SOCKS** 代理，而无需手动编辑分散在各处的配置文件。
...
也可单独执行 **`mpm use 作用域/preset`**（例如 **`mpm use docker/proxy`**）。**`proxy-group`** 默认顺序：**apt → shell → docker → k3s → go**（apt/docker/k3s 内部 sudo）。
...
| **`GATEWAY_IP`** | 按作用域解析的 Docker/CNI 网关 | `GATEWAY_IP` | `docker` / `k3s` preset 默认 |
...
# 内置走代理组合（apt → shell → docker → k3s → go；apt/docker/k3s 内部 sudo）
mpm use proxy-group
```
...
- **不**配置 **kind** 集群节点内代理（需 kind 集群 YAML；未来可能有 kind scope）。
